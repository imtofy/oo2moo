...
def _find_by_basename(vfs: Dict[str, bytes], name: str) -> Optional[bytes]:
    """Sucht eine Datei im Paket-VFS erst per exaktem Pfad, sonst per
    Dateiname - IMS-CP-Pakete referenzieren Dateien mal relativ zur
    HTML-Seite, mal nur als blanker Dateiname."""
    if name in vfs:
        return vfs[name]
...
def _process_page(href: Optional[str], sub_vfs: Dict[str, bytes], file_mgr, context_id: int,
                  ...
                  link_map: Optional[Dict]) -> Tuple[str, List[int], List[Dict], List[str]]:
    ...
    Gibt (bereinigtes HTML, File-IDs, entfernte Links, verwendete
    Dateinamen) zurück - Letztere braucht main.py für used_filenames,
    sonst tauchen Kapitel-Dokumente zusätzlich als 'verwaiste Dateien'
    ...
    if file_mgr is not None and asset_paths:
        ...
        # Dedup über den Dateinamen: sanitize_for_moodle liefert pro Bild-
        # VORKOMMEN einen Eintrag (zweimal referenziertes Bild = zwei
        # Einträge), und im selben Kapitel-Dateibereich darf jeder Dateiname
        # nur einmal registriert sein - doppelte Zeilen mit identischem
        # Schlüssel lehnt Moodle beim Restore sonst als Konflikt ab.
        seen_names = set()
        for asset_path in asset_paths:
            asset_filename = asset_path.split('/')[-1]
            if asset_filename in seen_names:
                continue
            seen_names.add(asset_filename)
            asset_data = _find_by_basename(sub_vfs, asset_path)
            ...
            used_names.append(asset_filename)

    return clean_html, file_ids, removed_links, used_names
...
def _generate_book_xml(module_id: int, context_id: int, title: str,
    ...
    for idx, ch in enumerate(chapters, start=1):
        safe_ch_title = html_lib.escape(ch['title'])
...
def build_fallback_book_xml(module_id: int, context_id: int, title: str,
    ...
    """
    warning = ('<p style="color:red;"><strong>Achtung:</strong> Der Inhalt dieses '
              'Content-Packages konnte nicht automatisch in Buch-Kapitel umgewandelt '
              'werden. Der ursprüngliche Text ist oben in der Beschreibung erhalten, '
              'der Rest muss manuell nachgetragen werden.</p>')
...
def build_book_activity(node: Dict, manifest, context_id: int, module_id: int, now: int,
    ...
    Gibt bei Erfolg {"book_xml", "file_ids", "removed_links",
    "used_filenames"} zurück, sonst None.
    """
    ...
    for page in pages:
        ...
        content, page_file_ids, page_removed_links, page_used_names = _process_page(
        ...
        used_filenames.extend(page_used_names)

    book_xml = _generate_book_xml(
        module_id=module_id, context_id=context_id,
        title=node.get('title', 'Buch'), intro_html=intro_html, now=now, chapters=chapters)
    ...
    result: BookActivityResult = {
        "book_xml": book_xml, "file_ids": file_ids,
        "removed_links": removed_links, "used_filenames": used_filenames,
    }
    return result
