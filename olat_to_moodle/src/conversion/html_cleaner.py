...
from config import EMBEDDED_CONTENT_LOST_WARNING, WARNING_SYMBOL
...
def _resolve_gotonode(tag, link_map):
    ...
    if not link_map:
        return False
    haystack = ' '.join(str(tag.attrs.get(attr_name, '')) for attr_name in ('href', 'onclick'))
...
def _image_gap_sides(tag) -> tuple:
    ...
    if cell is not None:
        ...
        if row is not None:
            ...
            if len(cells) > 1:
                idx = next((i for i, candidate in enumerate(cells) if candidate is cell), 0)
                if idx == 0:
                    return ('right',)
                if idx == len(cells) - 1:
                    return ('left',)
...
# OLAT markiert Formeln mit <span class="math">; das Attribut 'title' trägt
# dieselbe Formel zusätzlich URL-kodiert, maßgeblich ist aber der sichtbare
# Text. Die Klasse kann neben weiteren stehen, daher die Wortgrenzen.
_MATH_SPAN = re.compile(
    r'''<span[^>]*\bclass\s*=\s*(["'])[^"']*\bmath\b[^"']*\1[^>]*>(.*?)</span>''',
    re.DOTALL | re.IGNORECASE)
_ANY_TAG = re.compile(r'<[^>]+>')


def rewrite_math_formulas(html: str) -> str:
    """Ersetzt OLATs <span class="math">-Formeln durch Moodles LaTeX-
    Trennzeichen \\( … \\).

    OLAT erkennt Formeln am CSS-Klassennamen und rendert sie über MathJax;
    Moodle erkennt sie ausschließlich an den Trennzeichen (siehe
    filter/mathjaxloader). Ohne Umschreibung steht der LaTeX-Quelltext
    unverändert als sichtbarer Text im Kurs.

    Arbeitet bewusst auf dem rohen String statt über BeautifulSoup: die
    QTI-Kette reicht Fragetexte mit Interaktions-Elementen durch
    (<textEntryInteraction .../>), und ein HTML-Parser würde deren Namen
    kleinschreiben und aus dem Self-Closing-Tag ein Tag-Paar machen.

    Leere Container entfallen ersatzlos – '\\(\\)' wäre in Moodle ein
    sichtbarer Rest ohne Inhalt."""
    if not html or 'math' not in html:
        return html

    def to_delimiters(match):
        formula = _ANY_TAG.sub('', match.group(2)).strip()
        return f"\\({formula}\\)" if formula else ""

    return _MATH_SPAN.sub(to_delimiters, html)
...
def sanitize_for_moodle(raw_html: str, link_map: dict | None = None) -> tuple[str, list, list]:
    ...
    soup = BeautifulSoup(rewrite_math_formulas(raw_html), 'html.parser')
    extracted_assets = []
    removed_links = []
    ...
    for paragraph in soup.find_all('p'):
        text_content = paragraph.get_text(strip=True).replace('\xa0', '')
        if not text_content and not paragraph.find(['img', 'iframe', 'video', 'audio', 'figure', 'table']):
            paragraph.decompose()
    ...
    def _rewrite_asset_attr(el, attr_name):
        ...
        if value.startswith(('http://', 'https://', 'data:', '@@PLUGINFILE@@')
                             + _OLAT_INTERNAL_PATH_PREFIXES):
            return
    ...
    for tag in soup.find_all(True):
        ...
        if tag.name == 'iframe' and 'src' in tag.attrs:
            ...
            if parsed_src.netloc and _OLAT_SELF_REFERENCE_PATTERN.search(parsed_src.path):
                ...
                warning.string = EMBEDDED_CONTENT_LOST_WARNING.format(symbol=WARNING_SYMBOL)
        ...
        if tag.name == 'a':
            ...
            has_known_scheme = href.startswith(
                ('http://', 'https://', 'mailto:', 'tel:', '#', 'data:', '@@PLUGINFILE@@'))
            ...
            gotonode_attrs = ' '.join(str(tag.attrs.get(attr_name, '')) for attr_name in ('href', 'onclick'))
            is_course_internal = (
                not href
