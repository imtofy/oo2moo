...
def _validate_question_completeness(temp_dir: str, quiz_reports: list) -> None:
    ...
    for rep in quiz_reports:
        ...
        if not rep['resolved']:
            print(f"[FEHLER] Vollständigkeit: Test '{title}': QTI-Paket nicht auflösbar – "
        ...
        if actual_instances == -1:
            print(f"[FEHLER] Vollständigkeit: Test '{title}': quiz.xml fehlt oder ist "
                  f"nicht lesbar – Quiz-Inhalt nicht prüfbar.")
            continue

        if actual_instances != emitted:
            print(f"[FEHLER] Vollständigkeit: Test '{title}': Pipeline erzeugte {emitted} "
...
def _validate_question_bank(temp_dir: str, quiz_reports: list) -> None:
    ...
    try:
        ...
        print("[!] Vollständigkeit: questions.xml nicht lesbar.")
        return

    top_level = sum(1 for question in root.iter('question')
                    if (question.findtext('parent') or '').strip() == '0')
    expected = sum(rep['emitted'] for rep in quiz_reports)

    if top_level != expected:
        print(f"[FEHLER] Vollständigkeit: {expected} Fragen-Slot(s) über alle Quizze, aber "
...
def _validate_activity_files(temp_dir: str, processed_activities: list) -> None:
    ...
    if os.path.exists(backup_xml):
        try:
            ...
            print("[!] Vollständigkeit: moodle_backup.xml nicht lesbar.")
            return
    ...
    for act_id, m_type, _sec_id, title in processed_activities:
        ...
        if not os.path.exists(act_xml):
            print(f"[FEHLER] Vollständigkeit: Aktivität '{title}' ({m_type}_{act_id}) als gebaut "
                  f"geführt, aber {m_type}.xml fehlt auf der Platte.")
            continue
        if (m_type, str(act_id)) not in backup_activities:
            print(f"[FEHLER] Vollständigkeit: Aktivität '{title}' ({m_type}_{act_id}) fehlt in "
...
def validate_conversion_completeness(temp_dir: str, processed_activities: list,
    ...
    try:
        ...
        print(f"[FEHLER] Soll-Ist-Validierung mit unerwartetem Fehler abgebrochen: "
              f"{type(e).__name__}: {e}")
