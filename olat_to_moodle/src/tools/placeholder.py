...
with open(PLACEHOLDER_VIDEO_PATH, 'rb') as _f:
    _VIDEO_TEMPLATE = _f.read()
with open(PLACEHOLDER_PDF_PATH, 'rb') as _f:
    _PDF_TEMPLATE = _f.read()
with open(PLACEHOLDER_PPTX_PATH, 'rb') as _f:
    _PPTX_TEMPLATE = _f.read()
...
def make_pdf_placeholder(original_data: bytes) -> bytes:
    ...
    reader = PdfReader(io.BytesIO(_PDF_TEMPLATE))
    writer = PdfWriter()
...
def _trim_pptx(n: int) -> bytes:
    ...
    with zipfile.ZipFile(io.BytesIO(_PPTX_TEMPLATE)) as src:
